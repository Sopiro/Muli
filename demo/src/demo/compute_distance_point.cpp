...
public:
        ...
        : Demo(game)
    {
        ...
        if (r < 1.0f)
        {
            b = world->CreateRandomConvexPolygon(size / 2.0f, 10, identity, RigidBody::dynamic_body, minimum_radius);
        }
        else if (r < 2.0f)
        {
            b = world->CreateRandomConvexPolygon(size / 2.0f, 10, identity, RigidBody::dynamic_body, r / 10.0f + minimum_radius);
