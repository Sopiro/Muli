...
public:
    ...
    void CreateRagdoll(float headX, float headY, float scale)
    {
        ...
        // Arms
        {
            ...
            {
                float armMotorTorque = rightUpperArm->GetMass() * 3.0f * Sqrt(scale);
                float armMotorFrequency = 4.0f;
    ...
    {
        return new Bending(game);
    }
};

DemoFrame ragdoll{ "Ragdoll", Bending::Create };
