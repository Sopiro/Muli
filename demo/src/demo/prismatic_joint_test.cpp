...
public:
    ...
    PrismaticJointTest(Game& game)
        : Demo(game)
    {
        ground = world->CreateBox(100.0f, 0.4f, identity, RigidBody::Type::static_body);
        ...
        world->CreateCapsule(Vec2{ 3.0f, 4.8f }, Vec2{ 3.0f, 5.2f }, 0.1f, identity, RigidBody::Type::static_body);
        world->CreateCapsule(Vec2{ -3.0f, 4.8f }, Vec2{ -3.0f, 5.2f }, 0.1f, identity, RigidBody::Type::static_body);
    }
...
static int index = register_demo("Prismatic joint test", PrismaticJointTest::Create, 30);
